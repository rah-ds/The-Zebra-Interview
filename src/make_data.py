...
def clean_df(df: pd.DataFrame, save_local: bool = False) -> pd.DataFrame:
    ...
    df = df.assign(
        movie_title_clean=df["movie_title"].apply(
            lambda x: clean_and_normalize_movie_title(x)
        ),
        movie_franchise=df["movie_title"].apply(
            lambda x: x.split(":")[0] if ":" in x else np.nan
        ),
    )

    df = df.sort_values("num_users_voted", ascending=False)
    candidate_key_mask = df.duplicated(
        subset=["genres", "movie_title_clean", "movie_score"]
    )  # keep try of count
    df = df.drop_duplicates(
        subset=["genres", "movie_title_clean", "movie_score"], keep="first"
    )  # there can be multiple, want max user voted

    df = df.replace(0, np.nan)  # zeros are really nulls here
    df = df.assign(content_rating=df["content_rating"].replace("Not Rated", np.nan))
    df = df.assign(
        movie_type=np.where(
            df["content_rating"].isin(
    ...
    print(
        f"dropping the rows {candidate_key_mask.sum()} because they are duplicated - taking max audience score"
    )
...
def build_metrics(df_grp: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df.assign(
        ...
        ROI=(df["gross"] - df["budget"]) / df["budget"],  # profit / budget
    )

    # TODO: add in CAGR separately care needs to be made about the year
...
def build_cagr(df_grp: pd.DataFrame, year_col: str, metric_col: str) -> float:
    ...
    num_years = (
        df[year_col].iloc[-1] - df[year_col].iloc[0]
    )  # diff in years (end - start)
...
def filter_tv_shows(df) -> pd.DataFrame:
    ...
    assert "movie_type" in list(df), "run clean data first"
...
def get_long_text_summary(df: pd.DataFrame, piped_kw: str = None) -> pd.DataFrame:
    ...
    df_explode = (
        ...
        .sort_values("movie_score_median", ascending=False)
        .round(2)
    )
