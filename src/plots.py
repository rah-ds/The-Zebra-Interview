...
def plot_median_movie_score_over_time(df: pd.DataFrame) -> matplotlib.figure.Figure:
    ...
    df_tmp = df_tmp.assign(
        decade=df_tmp.decade.astype(str).str.replace("9990", "N/As")
    )  # make sure nulls on right side of graph

    tbl = (
        df_tmp.groupby("decade")
        .agg(
            n=("movie_title", "count"),
    ...
    sns.barplot(
        data=tbl, x="decade", y="n", ax=ax, color=colors_zebra_interview["Deep Blue"]
    ).set(title="Median Movie Score by Decade", ylabel="Count of Movies in Data")
    ...
    sns.pointplot(
        ...
        label="Median Movie Rating (1-10) by decade",
    ).set(ylabel="Median Score (1-10)")

    ax2.set_ylim(1, 10)
    plt.axhline(
        y=tbl["median_score"].median(),
        color=colors_zebra_interview["Light Gray"],
        linestyle="--",
        linewidth=1.5,
        label="Median Overall",
    )  # doesn't make sense take from the raw
...
def plot_median_movie_lift_over_time(df: pd.DataFrame) -> matplotlib.figure.Figure:
    ...
    fig, ax = plt.subplots()
    tbl = tbl.assign(
        decade=tbl.decade.astype(str).str.replace("9990", "N/As")
    )  # make sure nulls on right side of graph

    sns.barplot(
        data=tbl, x="decade", y="n", ax=ax, color=colors_zebra_interview["Deep Blue"]
    ).set(
        title="Median Movie Profit (lift) by Decade", ylabel="Count of Movies in Data"
    )
    ...
    sns.pointplot(
        ...
        label="lift",
    ).set(ylabel="lift = (Gross / Budget)")

    plt.axhline(
        y=tbl["lift"].median(),
        color=colors_zebra_interview["Warm Gray"],
        linestyle="--",
        linewidth=1.5,
        label="Median Overall",
    )  # doesn't make sense take from the raw
